      ...
      if (user) {
        try {
          const response = await axios.get(
            `http://localhost:3000/dashboard/activities-count-by-month-user`,
            {
              params: {
                year,
                userId: user.user_id,
          ...
          console.error("Failed to fetch activities count by month:", error);
        }
      }
    };
    fetchData();
...
const MonthlyActivitiesCard = () => {
  ...
  useEffect(() => {
    ...
    const fetchTotalActivities = async () => {
      if (user) {
        try {
          const response = await axios.get(
            `http://localhost:3000/dashboard/total-activities-user`,
            {
              params: {
                userId: user.user_id,
              },
          ...
          console.error("Failed to fetch total activities for user:", error);
        }
      }
    };
  ...
  return (
    ...
    >
      <Typography>Your Monthly Activities</Typography>
