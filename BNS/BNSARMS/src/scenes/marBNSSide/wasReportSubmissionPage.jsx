...
import {
  submitWorkActivityScheduleReport,
  uploadFile,
  submitFeedback,
  getWorkActivityScheduleReportDetails,
  updateFile,
  deleteReport,
...
const ReportSubmissionPage = () => {
  ...
  useEffect(() => {
    if (user) {
      const fetchReport = async () => {
        const details = await getWorkActivityScheduleReportDetails(
          month,
          year,
          user.user_id
        );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!reportData) {
        reportData = await submitWorkActivityScheduleReport(
          user.user_id,
          month,
          parseInt(year, 10)
        );
  ...
  const handleDelete = async () => {
    if (existingFile && existingFile.reportId) {
      try {
        ...
        navigate(`/app/was-select-month/${year}/${userRole}`); // Now userRole is defined here
      } catch (error) {
  ...
  return (
    <Box sx={{ p: 4 }}>
      <Box display="flex" justifyContent="space-between" alignItems="center">
        <Header
          title="Submit Work Activity Schedule Report"
