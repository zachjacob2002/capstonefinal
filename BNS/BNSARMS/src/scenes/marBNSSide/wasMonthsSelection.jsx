...
// workActivityMonthsSelection.jsx
import { Card, CardActionArea, Typography, Box } from "@mui/material";
import { useNavigate } from "react-router-dom";
import { useState, useEffect } from "react";
import {
  getWorkActivityReportDetails,
  getWorkActivitySubmissionCount,
...
const MonthSelection = ({ month, year, isNPC }) => {
    ...
    if (user && monthIndex <= currentMonth) {
      ...
      if (isNPC) {
        const fetchSubmissionCount = async () => {
          const countDetails = await getWorkActivitySubmissionCount(
            month,
            year
          );
        ...
        const fetchReportDetails = async () => {
          const details = await getWorkActivityReportDetails(
            month,
            year,
            user?.user_id
  ...
  const handleCardClick = () => {
    if (user && monthIndex <= currentMonth) {
      // Check if user exists
      if (user.role == 2) {
        navigate(`/app/was-submissions/${month}/${year}`);
      } else if (user.role == 1) {
        navigate(`/app/was-reports/${month}/${year}`);
      }
    }
  };
